...
lookupPackageConfig unitId env = lookupPackage' False pkgConfigMap unitId
 where
  pkgConfigMap =
...
runGhcEnv env act = do
  ...
  let dflags = (hsc_dflags env) { filesToClean = filesToClean
...
cgGutsToCoreModule safeMode guts modDetails =
  CoreModule (cg_module guts) (md_types modDetails) (cg_binds guts) safeMode
...
hDuplicateTo' h1 _ = ioe_dupHandlesNotCompatible h1
...
dupHandleTo
  ...
  -> IO Handle__
dupHandleTo filepath h other_side _hto_@Handle__ { haDevice = devTo } h_@Handle__ { haDevice = dev } mb_finalizer
  = do
    flushBuffer h_
    case cast devTo of
...
dupHandle_ new_dev filepath other_side _h_@Handle__ {..} mb_finalizer = do
   -- XXX wrong!
  mb_codec <- if isJust haEncoder
    then fmap Just getLocaleEncoding
    else return Nothing
  mkHandle new_dev
           filepath
           haType
           True{-buffered-}mb_codec
...
ioe_dupHandlesNotCompatible h = ioException
  (IOError (Just h)
           IllegalOperation
           "hDuplicateTo"
